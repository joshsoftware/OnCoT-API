...
module Api
  module V1
    class ExecutionsController < ApiController
      def submission_token # rubocop:disable Metrics/AbcSize
      ...
      def submission_result # rubocop:disable Metrics/AbcSize
        ActionCable.server.broadcast(
          params[:room],
          {
            stdout: Base64.decode64(params[:stdout].to_s),
            time: params[:time], memory: params[:memory],
            stderr: Base64.decode64(params[:stderr].to_s), token: params[:token],
            compile_output: Base64.decode64(params[:compile_output].to_s),
            message: Base64.decode64(params[:message].to_s), status: params[:status]
          }
        )
      end
