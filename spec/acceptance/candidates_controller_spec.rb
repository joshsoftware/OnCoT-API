...
resource 'Candidates' do 
  explanation 'Candidates resource'

  let(:drive) { Factory(:drive) }
  let(:candidate) { Factory(:candidate) }
  let(:id) { Factory(:id) }
  let(:drife_id) { drive.id }
 
  
  put '/api/v1/candidates/:id' do
    ...
    context 'with valid params' do
      let(:id) { 1 }
      example 'updates the particular candidate details' do
        request = {
            data: {
              id: 1,
              drife_id: 1,
